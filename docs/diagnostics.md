...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/presentation-commander-client/` |
| Linux | `$XDG_STATE_HOME/presentation-commander-client/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\presentation-commander-client\logs\` |

`PC_CLIENT_LOG_DIR` overrides it. The path is printed on the first line of every
...
`presentation-commander-client.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `PC_CLIENT_LOG`, default `info`.
...
```bash
presentation-commander-client --collect-diagnostics
```

Writes `presentation-commander-client-diagnostics-<timestamp>.json` and prints its path. One file, so
