# Running unsigned Presentation Commander Client builds
...
Presentation Commander Client listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of Presentation Commander Client**
...
domain. Presentation Commander Client needs this to accept control from the Master Server and send its Program and Next-Slide NDI outputs.

If you deny it (or the prompt appears behind another window and times out), the Server won't discover it and no NDI outputs will appear downstream
...
**Private**. If Presentation Commander Client isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "Presentation Commander Client" -Direction Inbound -Program "C:\Path\To\Presentation Commander Client.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*Presentation Commander Client*" | Remove-NetFirewallRule
```
...
```sh
chmod +x ./presentation-commander-client
```
...
```sh
codesign -dv --verbose=4 "/Applications/presentation-commander-client.app"
spctl -a -vv "/Applications/presentation-commander-client.app"   # still reports "rejected" — ad-hoc is not notarization
```
